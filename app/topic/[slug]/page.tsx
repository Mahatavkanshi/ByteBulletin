  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="border-b border-border pb-4">
        ...
        <h1 className="mt-2 text-4xl">{topicData.topic.name}</h1>
      ...
      <section className="mt-8 grid gap-8 lg:grid-cols-[2fr_1fr]">
        <div className="space-y-4">
          ...
          {topicData.timeline.length === 0 ? (
            <p className="text-sm text-muted">Timeline updates are being prepared.</p>
          ) : (
            topicData.timeline.map((event) => (
              <article key={event.slug} className="rounded-lg border border-border bg-surface p-4 shadow-sm">
                <p className="text-xs font-semibold uppercase tracking-wider text-brand">{event.eventAt}</p>
                <h3 className="mt-2 text-xl leading-tight">{event.title}</h3>
                <p className="mt-2 text-sm text-[#2d4152]">{event.update}</p>
        ...
        <aside className="space-y-4 rounded-xl border border-border bg-surface p-5">
          <h2 className="text-xl">Related Coverage</h2>
          {relatedStories.length === 0 ? (
            <p className="text-sm text-muted">Related stories will appear here soon.</p>
          ) : (
            relatedStories.map((story) => (
              <article key={story.slug} className="border-b border-border pb-3 last:border-none">
                ...
                <p className="mt-1 text-xs text-muted">{story.publishedAt}</p>
