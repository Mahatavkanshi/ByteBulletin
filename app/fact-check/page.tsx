  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="border-b border-border pb-4">
        ...
        <h1 className="mt-2 text-4xl">Fact Check</h1>
      ...
      <section className="mt-8 grid gap-5 md:grid-cols-2">
        {checks.map((check) => (
          <article key={check.slug} className="rounded-xl border border-border bg-surface p-5 shadow-sm">
            ...
            <h2 className="mt-3 text-2xl leading-tight">{check.title}</h2>
            <p className="mt-2 text-sm text-muted">Claim: {check.claim}</p>
            <p className="mt-3 text-sm text-[#2c3f50]">{check.summary}</p>
            <p className="mt-3 text-sm text-muted">{check.explanation}</p>
